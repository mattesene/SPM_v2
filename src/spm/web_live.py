...
def render_live_dashboard(
    ...
    *,
    ...
    for index, item in enumerate(rows, 1):
        ...
        selected = escape(item.selected_team or home)
        cards.append(f'''<article class="card {'featured' if index == 1 else ''}">
<header><div class="rank">#{index}</div><div class="label">SQUADRA DA SEGUIRE</div></header>
<section class="selection"><small>SCELTA PER LA SERIE DI PAREGGI</small><strong>{selected}</strong><span>Probabilità squadra: <b>{item.team_probability:.1%}</b></span></section>
...
<div class="scores"><div><small>SPM TEAM SCORE</small><b>{item.spm_score:.1f}</b></div><div><small>SERIE ATTUALE</small><b>{item.team_streak}</b></div><div><small>X DOPO SERIE</small><b>{item.streak_draw_rate:.1%}</b></div><div><small>OOS SCORE</small><b>{oos_score}</b></div></div>
<div class="evidence"><span>Qualità OOS <b class="tag {oos_class}">{oos_quality}</b></span><span>Finestre profittevoli <b>{rate}</b></span><span>Segnale <b>PAREGGIO</b></span></div>
    ...
    body = ''.join(cards) or '<div class="empty">Nessuna selezione live disponibile — Nessuna squadra candidata disponibile</div>'
