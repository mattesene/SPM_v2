...
def _oos_quality(item: LiveCandidate) -> tuple[str, str]:
...
def render_live_dashboard(candidates: Iterable[LiveCandidate], *, as_of: str, live_status: str = "LIVE AGGIORNATO") -> str:
    ...
    for index, item in enumerate(rows, 1):
        ...
        oos_quality, oos_class = _oos_quality(item)
...
<div class="evidence"><span>Qualità OOS <b class="tag {oos_class}">{oos_quality}</b></span><span>Finestre profittevoli <b>{item.profitable_window_rate:.0%}</b></span><span>Segnale <b>PAREGGIO</b></span></div>
</article>''')
