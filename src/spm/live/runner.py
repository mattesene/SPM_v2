...
def run_live_from_database(db_path: str | Path, oos_entries: list[OOSRankingEntry], *, as_of: date, output: str | Path, oos_path: str | Path | None = None, max_match_age_days: int | None = 14) -> tuple:
    ...
    print(f"live_inputs,as_of={as_of.isoformat()},matches={len(matches)},fixtures={len(fixtures)},fixture_dates={[fixture.date.isoformat() for fixture in fixtures]}")
    print("live_fixture_rows=" + repr([(fixture.home_team, fixture.away_team, fixture.date.isoformat()) for fixture in fixtures]))
    quality = assess_live_data(matches, fixtures, as_of=as_of, max_match_age_days=max_match_age_days)
    if not quality.ok:
        details = "; ".join(quality.warnings) or "unknown data-quality failure"
        raise RuntimeError(f"Live data quality gate failed: {details}")
    candidates = build_upcoming_live_report(matches, fixtures, oos_entries, as_of=as_of, path=output)
