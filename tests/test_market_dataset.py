...
def _match() -> MatchRecord:
    return MatchRecord(date=date(2025, 1, 1), home_team="Milan", away_team="Roma", home_goals=1, away_goals=1)


def test_attach_draw_odds_to_normalized_match():
    result = attach_draw_odds([_match()], [DrawOdds(date(2025, 1, 1), "Milan", "Roma", 3.2)])
